...
var SensAppStorage = function(superOperator, statement_name, resume)
{
	...
	this.resume = resume = resume.substr(8).split('|');

	if (this.resume.length > 1) {
		var data = {};
		for (var i = 0; i < this.resume.length-1;++i)
		{
			var url = this.resume[i+1];
			if (url.substr(0, 7) !== 'http://' && url.substr(0, 8) !== 'https://')
				url = "http://demo.sensapp.org:80/sensapp/databases/raw/data/"+url;

			data[this.resume[i++]] = url;
		}
		this.additional_data = data;
	}
	else
	{
		var url = this.resume[0];
		if (url.substr(0, 7) !== 'http://' && url.substr(0, 8) !== 'https://')
			url = "http://demo.sensapp.org:80/sensapp/databases/raw/data/"+url;

		this.additional_data = {value: url};
	}
