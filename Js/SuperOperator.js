...
var SuperOperator = function() {
	...
	// store idm-inspector sensors informations
	this.sensors_informations = {};
...
add_statement: function(d, obj) {
	...
	else
	{
		...
		var url = obj.sensors_informations[statement_name];

		if (url)
		{
			if (url.substr(0, 8) === 'sensapp:')
			{
				var storage = 'SensAppStorage';
			}
			else if (url.substr(0,6) === 'video:')
			{
			...
			if (typeof window[storage] !== 'undefined')
			{
				...
				var storageInstance = new window[storage](obj, statement_name, url);
			...
			EventBus.send('error', {status: statement_name, message: 'Unable to load sensors informatinos'});
		}
	}
},
